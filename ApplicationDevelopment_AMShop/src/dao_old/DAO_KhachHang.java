...
public class DAO_KhachHang extends DAO {
	...
	public static boolean updateKhachHang(KhachHang khachHang) {
		int n = 0;
		try {
			String sql = "" + "UPDATE KhachHang " + "SET " + "HoTen = ?, " + "SoDienThoai = ?, "
					+ "DiaChi = ?, " + "NhomKhachHang = ? " + "WHERE MaKhachHang = ?";
			PreparedStatement prs = connection.prepareStatement(sql);
	...
	public static ArrayList<KhachHang> getAllKhachHang() {
		...
		try {
			...
			while (rs.next()) {
				...
				KhachHang khachHang = new KhachHang(maKhachHang, hoTen, soDienThoai, diaChi,
						nhomKhachHang);
				list.add(khachHang);
			}
		} catch (SQLException ex) {
	...
	public static ResultSet thongKeKhachHangTheoKhoangNgay(LocalDate ngayBatDau,
		...
		try {
			String sql = ""
					...
					+ "WHERE HD.ThoiGianTao BETWEEN ? AND ? " + "GROUP BY HD.MaKhachHang";
			PreparedStatement prs = connection.prepareStatement(sql);
	...
	public static double getSoTienKhachHangDaThanhToanTheoMaKhachHang(String maKhachHang) {
		double d = 0;
		try {
			String sql = "" + "SELECT SUM(CTHD.DonGia) AS TongTienDaMua "
					+ "FROM (HoaDon HD JOIN ChiTietHoaDon CTHD ON HD.MaHoaDon = CTHD.MaHoaDon) JOIN KhachHang KH ON HD.MaKhachHang = KH.MaKhachHang "
					+ "WHERE KH.MaKhachHang = ?" + "GROUP BY HD.MaKhachHang";
			PreparedStatement prs = connection.prepareStatement(sql);
			prs.setString(1, maKhachHang);
